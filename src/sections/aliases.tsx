...
function AliasesPage() {
  ...
  useEffect(() => {
    if (loginData.ip && loginData.apiKey) {
      dispatch(actionsAsync.getAliasesThunk({ apiEndpoint: loginData.ip, apiToken: loginData.apiKey }))
    }
  ...
  return (
    ...
    >
      ...
      <CSVUploader onParse={(parsedData) => {
        for (const data of parsedData) {
          if (data.alias && data.peerId && loginData.ip && loginData.apiKey) {
            dispatch(actionsAsync.setAliasThunk({
...
function CreateAliasForm() {
  ...
  return (
    <div>
      <input type="text" name="peerId" placeholder="peerId" onChange={handleChange} value={form.peerId} />
