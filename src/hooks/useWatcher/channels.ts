 ...
 */
export const checkHowChannelsHaveChanged = (previousChannels: ChannelsParsed, newChannels: ChannelsParsed) => {
  const previousChannelsLocal = JSON.parse(JSON.stringify(previousChannels)) as ChannelsParsed;
  const newChannelsLocal = JSON.parse(JSON.stringify(newChannels)) as ChannelsParsed;

  const changes: ChannelOutgoingType[] = [];

  const previousChannelsOutgoing = Object.keys(previousChannelsLocal.outgoing);
  const newChannelsOutgoing = Object.keys(newChannelsLocal.outgoing);
